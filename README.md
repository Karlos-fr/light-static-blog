...
│       ├── mosaic/theme.css    # Thème public en grille graphique
...
Trois thèmes publics sont fournis : `default` pour un rendu simple et polyvalent, `folio` pour un rendu éditorial plus sobre, et `mosaic` pour une grille d'articles plus graphique. Les thèmes personnels peuvent rester ignorés par Git.
...
Le script lance le build sauf avec `-SkipBuild`, contrôle RSS/sitemap/canonical/JSON-LD, transfère `dist/`, puis contrôle les URL publiques. Par défaut, `-SyncMode Full` transfère tous les fichiers générés ; `-SyncMode Diff` compare les empreintes SHA-256 locales avec un manifeste distant `.light-static-blog-deploy-manifest.json` et n'envoie que les fichiers absents ou modifiés. Ce mode évite de relire tous les fichiers distants à chaque déploiement et reste donc rapide après le premier passage. `-VerifyRemoteHashes` force une relecture SHA-256 complète du serveur distant pour détecter et corriger une dérive manuelle. `-ManifestRemotePath` permet de personnaliser le chemin du manifeste. La sortie détaillée d'Astro est masquée par défaut pour garder un rendu console propre ; `-VerboseBuild` permet de l'afficher. Si le blog est servi dans un sous-chemin, `-RootRobotsRemotePath` permet aussi de publier `robots.txt` à la racine de l'hôte.
